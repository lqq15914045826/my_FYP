...
for method_dir in method_dirs:
    ...
    ckpt_root_dir = os.path.join(
        syncheck_cond_dir,
        method_dir,
        f"unconditional_{method_name}_syncheck_checkpoints",
    )
    ckpt_dirs = os.listdir(ckpt_root_dir)
    uncond_acc_list = []
    for ckpt_dir in ckpt_dirs:
    ...
    ckpt_root_dir = os.path.join(
        syncheck_cond_dir, method_dir, f"condlabel_{method_name}_syncheck_checkpoints"
    )
    ckpt_dirs = os.listdir(ckpt_root_dir)
    condlabel_acc_list = []
    for ckpt_dir in ckpt_dirs:
...
ax.set_xticklabels(("跨域迁移", "域内扩增"), fontsize=18)
...
# plt.show()
plt.savefig(
    os.path.join(save_dir, "condition_impact.pdf"), dpi=800, bbox_inches="tight"
)
