...
for model_name in ["resnet18", "resnet50"]:
    ckpt_root_dir = os.path.join(
        baseline_dir, method_dir, f"{method_name}_reproduce_checkpoints_{model_name}"
    )
    ckpt_dirs = os.listdir(ckpt_root_dir)
    mix_acc_list = []
    for ckpt_dir in ckpt_dirs:
    ...
    ckpt_root_dir = os.path.join(
        baseline_dir, method_dir, f"{method_name}_filterssim_checkpoints_{model_name}"
    )
    ckpt_dirs = os.listdir(ckpt_root_dir)
    filterssim_acc_list = []
    for ckpt_dir in ckpt_dirs:
    ...
    # filtertrts
    ckpt_root_dir = os.path.join(
        baseline_dir, method_dir, f"{method_name}_filtertrts_checkpoints_{model_name}"
    )
    ckpt_dirs = os.listdir(ckpt_root_dir)
    filtertrts_acc_list = []
    for ckpt_dir in ckpt_dirs:
    ...
    # SynCheck
    ckpt_root_dir = os.path.join(
        syncheck_dir, method_dir, f"{method_name}_syncheck_checkpoints_{model_name}"
    )
    ckpt_dirs = os.listdir(ckpt_root_dir)
    syncheck_acc_list = []
    for ckpt_dir in ckpt_dirs:
...
# plt.show()
plt.savefig(
    os.path.join(save_dir, f"backbone_{method_name}.pdf"), dpi=800, bbox_inches="tight"
)
