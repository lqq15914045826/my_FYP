...
for seed in [1223, 213, 420, 219, 307]:
    subprocess.run(["python", "ablation_rfdiffusion.py", \
        "--seed", str(seed), "--normalize_dfs", \
        "--lambda_ova", str(0.0), 
        "--checkpoint_root_dir", "rfdiffusion_ablation_checkpoints_woova"])

ckpt_root_dir = "rfdiffusion_ablation_checkpoints_woova"
...
for seed in [1223, 213, 420, 219, 307]:
    subprocess.run(["python", "ablation_rfdiffusion.py", \
        ...
        "--checkpoint_root_dir", "rfdiffusion_ablation_checkpoints_woconsent"])
ckpt_root_dir = "rfdiffusion_ablation_checkpoints_woconsent"
ckpt_dirs = os.listdir(ckpt_root_dir)
woconsent_acc_list = []
for ckpt_dir in ckpt_dirs:
    model_ckpts = [f for f in os.listdir(os.path.join(ckpt_root_dir, ckpt_dir)) if f.endswith('.pth')]
