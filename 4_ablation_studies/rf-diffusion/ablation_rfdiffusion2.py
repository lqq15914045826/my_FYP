    ...
    for epoch in range(args.epochs):
        if epoch >= args.start_fix:
            ...
            if len(unlabeled_dataset) == 0:
                logger.warning(f"Epoch {epoch}: empty unlabeled dataset after exclusion, skip FixMatch this epoch")
                unlabeled_dataset = unlabeled_dataset_all  # 回退到原始
...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'{seed_name}-{data_name}-{args.model_name}-{threshold_name}-{temp_name}-{weight_name}-{CURRENT_TIME}')
...
if __name__ == '__main__':
    ...
    # log parameters
    parser.add_argument('--log_dir', type=str, default='./rfdiffusion_ablation_logs', help='path to log directory')
    parser.add_argument('--checkpoint_root_dir', type=str, default='./rfdiffusion_ablation_checkpoints', help='path to checkpoint directory')
