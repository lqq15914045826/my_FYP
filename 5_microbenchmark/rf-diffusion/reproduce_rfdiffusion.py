...
def main(args):
    ...
    if args.syn_ratio != 1.0:
        ...
        syn_train_dataset = SynDFSDataset(args.ori_syn_data_dir, args.ori_syn_fname_label_dir, 'train', 
            args.train_rxs, args.normalize_dfs, return_ids=True)
    concat_train_dataset = ConcatDataset([real_train_dataset, syn_train_dataset])
    real_train_loader = DataLoader(real_train_dataset, batch_size=args.batch_size, shuffle=True)
    concat_train_loader = DataLoader(concat_train_dataset, batch_size=args.batch_size, shuffle=True)
    ...
    for epoch in range(args.epochs):
        if epoch >= args.warmup_epochs:
            train_loader = concat_train_loader
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--warmup_epochs', type=int, default=3, help='Number of epochs to warmup the model.')
    
    # log parameters
    parser.add_argument('--log_dir', type=str, default='./rfdiffusion_reproduce_logs', help='Path to the directory to save logs.')
    parser.add_argument('--checkpoint_root_dir', type=str, default='./rfdiffusion_reproduce_checkpoints', help='Path to the directory to save checkpoints.')
