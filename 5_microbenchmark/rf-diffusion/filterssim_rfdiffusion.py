...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, f'{ratio_name}-ssim{args.ssim_threshold}-{args.model_name}-seed{args.seed}-{CURRENT_TIME}')
    ...
    if args.syn_ratio != 1.0:
        ...
        syn_train_dataset = SynDFSDataset(args.ori_syn_data_dir, args.ori_syn_fname_label_dir, 'train', 
            args.train_rxs, args.normalize_dfs, return_ids=True)
    logger.info(f"Real train dataset size: {len(real_train_dataset)}, Synthetic train dataset size: {len(syn_train_dataset)}")
    
    # filter syn_dataset with ssim to real data
    syn_cnt = len(syn_train_dataset)
    real_cnt = len(real_train_dataset)
    
    if args.syn_ratio != 1.0:
        ssim_scores = []
        for si in tqdm(range(syn_cnt)):
            syn_csi = syn_train_dataset[si][0].numpy()
            if args.syn_ratio != 1.0:
                cur_idx = syn_train_dataset.selected_indices[si]
            else:
                cur_idx = si
            file_idx = cur_idx // len(syn_train_dataset.rx_list)
            rx_idx = cur_idx % len(syn_train_dataset.rx_list)
            syn_filename = syn_train_dataset.records[file_idx]
            if args.syn_ratio != 1.0:
                syn_filename = '-'.join(syn_filename.split('-')[:-1])
            best_ssim_score = 0.0
            for ri in range(real_cnt):
                real_file_idx = ri // len(real_train_dataset.rx_list)
                real_rx_idx = ri % len(real_train_dataset.rx_list)
                real_filename = real_train_dataset.records[real_file_idx]
                if real_filename in syn_filename and real_rx_idx == rx_idx:
                    real_csi = real_train_dataset[ri][0].numpy()
                    ssim_score = real_ssim(syn_csi, real_csi)
                    if ssim_score > best_ssim_score:
                        best_ssim_score = ssim_score
            ssim_scores.append(best_ssim_score)
        ssim_scores = np.array(ssim_scores)
        logger.info(ssim_scores)
