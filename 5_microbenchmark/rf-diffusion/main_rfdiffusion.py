...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'{ratio_name}-{seed_name}-{data_name}-{args.model_name}-{threshold_name}-{temp_name}-{weight_name}-{CURRENT_TIME}')
    ...
    if args.syn_ratio != 1.0:
        all_syn_dataset = SynDFSDataset(args.syn_data_dir, args.syn_fname_label_dir, 'train', 
            args.train_rxs, args.normalize_dfs, return_ids=True)
        selected_record_cnt = int(args.syn_ratio * len(all_syn_dataset) / 5) // 3
        np.random.seed(args.seed)
        selected_indices = np.random.choice(len(all_syn_dataset.records), selected_record_cnt, replace=False)
        selected_fname_label_dir = os.path.join(args.syn_fname_label_dir, f'{args.syn_ratio}_{args.seed}')
        os.makedirs(selected_fname_label_dir, exist_ok=True)
        new_filename = all_syn_dataset.records[selected_indices]
        new_labels = all_syn_dataset.labels[selected_indices]
        np.save(os.path.join(selected_fname_label_dir, 'train_filename.npy'), new_filename)
        np.save(os.path.join(selected_fname_label_dir, 'train_label.npy'), new_labels)
        unlabeled_dataset = SynDFSDataset(args.syn_data_dir, selected_fname_label_dir, 'train', \
            args.train_rxs, args.normalize_dfs, syn_transform)
    else:
        unlabeled_dataset = SynDFSDataset(args.ori_syn_data_dir, args.ori_syn_fname_label_dir, 'train', \
            args.train_rxs, args.normalize_dfs, syn_transform)
...
if __name__ == '__main__':
    ...
    # log parameters
    parser.add_argument('--log_dir', type=str, default='./rfdiffusion_syncheck_logs', help='path to log directory')
    parser.add_argument('--checkpoint_root_dir', type=str, default='./rfdiffusion_syncheck_checkpoints', help='path to checkpoint directory')
