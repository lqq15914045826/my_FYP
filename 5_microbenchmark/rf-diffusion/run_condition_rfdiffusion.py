...
for seed in seed_list:
    subprocess.run(["python", "unconditional_main_rfdiffusion.py", \
        "--seed", str(seed), "--normalize_dfs"])

ckpt_root_dir = "unconditional_rfdiffusion_syncheck_checkpoints"
...
for seed in seed_list:
    subprocess.run(["python", "condlabel_main_rfdiffusion.py", \
        "--seed", str(seed), "--normalize_dfs"])

ckpt_root_dir = "condlabel_rfdiffusion_syncheck_checkpoints"
ckpt_dirs = os.listdir(ckpt_root_dir)
syncheck_acc_list = []
for ckpt_dir in ckpt_dirs:
    model_ckpts = [f for f in os.listdir(os.path.join(ckpt_root_dir, ckpt_dir)) if f.endswith('.pth')]
