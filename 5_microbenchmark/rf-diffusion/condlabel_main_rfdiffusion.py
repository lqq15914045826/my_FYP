    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            try:
                ...
                (_, inputs_x_s, inputs_x), targets_x = labeled_iter.__next__()
            try:
                (inputs_u_w, inputs_u_s, _), targets_unlabeled = unlabeled_iter.__next__()
            except:
                unlabeled_iter = iter(unlabeled_loader)
                (inputs_u_w, inputs_u_s, _), targets_unlabeled = unlabeled_iter.__next__()
            try:
            ...
            if epoch >= args.start_fix:
                ...
                L_fix = F.cross_entropy(logits_u_s, targets_unlabeled.to(device), reduction='none').mean()
            else:
...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'condlabel-{seed_name}-{data_name}-{args.model_name}-{threshold_name}-{temp_name}-{weight_name}-{CURRENT_TIME}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--syn_data_dir', type=str, default='../../1_synthetic_data_generation/rf-diffusion/syn_data_native', help='Path to the directory containing the synthetic data.')
    ...
    # log parameters
    parser.add_argument('--log_dir', type=str, default='./condlabel_rfdiffusion_syncheck_logs', help='path to log directory')
    parser.add_argument('--checkpoint_root_dir', type=str, default='./condlabel_rfdiffusion_syncheck_checkpoints', help='path to checkpoint directory')
