...
def train(
    ...
    unlabeled_real_loader = DataLoader(
        unlabeled_real_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            unlabeled_real_loss = -args.unlabeled_real_lambda * torch.mean(
                torch.mul(unlabeled_pred, unlLabel)
            )
            vanilla_gen_loss = -args.vanilla_gen_lambda * torch.mean(
                torch.mul(vanilla_gen_pred, genLabel)
            )
            # print("vanilla_gen_pred: ", vanilla_gen_pred[0], vanilla_gen_loss.item())
            # print("syn_pred: ", syn_logits[0], syn_target[0], syn_loss.item())
            unlabeled_loss = (
                unlabeled_real_loss + args.cycle_lambda * syn_loss + vanilla_gen_loss
            )

            # overall discriminator loss
            loss_dis = (
                args.unlabeled_weight * unlabeled_loss
                + args.labeled_weight * labeled_loss
            )
            loss_dis.backward()
            optimizer_dis.step()
            ...
            if batch_idx % args.log_interval == 0:
                ...
                writer.add_scalar(
                    "train/loss_unlabeled_real",
                    unlabeled_real_loss.item(),
                    epoch * args.eval_step + batch_idx,
                )
                writer.add_scalar(
                    "train/loss_cycle_syn",
                    syn_loss.item(),
                    epoch * args.eval_step + batch_idx,
                )
                writer.add_scalar(
                    "train/loss_vanilla_gen",
                    vanilla_gen_loss.item(),
                    epoch * args.eval_step + batch_idx,
                )
                logger_message = (
                    f"Train Epoch: {epoch} [{batch_idx*args.batch_size}/{args.eval_step*args.batch_size} ({100. * batch_idx / args.eval_step:.0f}%)]"
        ...
        val_acc = test(
            test_loader, model, epoch, device, logger, writer, args.category_num // 2
        )
...
def main(args):
    ...
    checkpoint_dir = os.path.join(
        ...
        f"{ratio_name}-{data_name}-{model_name}-{seed_name}-{weight_name}-{CURRENT_TIME}",
    )
    ...
    scheduler_dis = optim.lr_scheduler.StepLR(
        optimizer_dis, step_size=args.lr_decay_step, gamma=args.lr_decay_rate
    )
    optimizer_gen = optim.Adam(generator.parameters(), lr=args.lr)
    scheduler_gen = optim.lr_scheduler.StepLR(
        optimizer_gen, step_size=args.lr_decay_step, gamma=args.lr_decay_rate
    )

    # set up dataset
    labeled_dataset = SignFiCSIDataset(
        args.dataset_dir, "train_csi", "train_label", args.normalize_csi
    )
    ...
    # first train the model without synthetic data
    # half the epochs: as we train the model twice
    ...
    filter_indices = []
    with torch.no_grad():
        syn_loader = DataLoader(syn_dataset, batch_size=args.batch_size, shuffle=False)
        for batch_idx, (inputs, targets) in enumerate(tqdm(syn_loader)):
            inputs, targets = inputs.to(device), targets.to(device)
            outputs, _ = model(inputs)
            _, predicted = outputs.max(1)
    ...
    logger.info(f"Select {len(syn_dataset)} synthetic data with TRTS")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--syn_dataset_dir",
        type=str,
        default="more_synthetic_data",
        help="path to original data",
    )
    parser.add_argument("--normalize_csi", action="store_true", help="normalize csi")
    parser.add_argument(
        "--category_num", type=int, default=101, help="number of classes"
    )
    parser.add_argument(
        "--syn_ratio",
        type=float,
    ...
    parser.add_argument(
        "--log_dir",
        type=str,
        default="./csigan_filtertrts_logs",
        help="path to log directory",
    )
    parser.add_argument(
        "--checkpoint_root_dir",
        type=str,
        default="./csigan_filtertrts_checkpoints",
    ...
    # loss parameters
    parser.add_argument(
        "--labeled_weight", type=float, default=1.0, help="labeled data weight"
    )
    parser.add_argument(
        "--unlabeled_weight", type=float, default=1.0, help="unlabeled data weight"
    )
    parser.add_argument(
        "--unlabeled_real_lambda",
        type=float,
        default=0.6,
        help="lambda for unlabeled real data",
    )
    parser.add_argument(
        "--cycle_lambda",
        type=float,
        default=0.1,
        help="lambda for cycle consistency loss",
    )
    parser.add_argument(
        "--vanilla_gen_lambda",
        type=float,
        default=0.1,
        help="lambda for vanilla generator loss",
    )
