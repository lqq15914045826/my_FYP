...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'unconditional-{ablation_name}-{seed_name}-{data_name}-{model_name}-{threshold_name}-{temp_name}-{CURRENT_TIME}')
    ...
    model = Discriminator_syncheck(category=args.num_classes, mid_channels=args.mid_channels, seed=args.seed)
    model.to(device)
    ...
    unlabeled_syn_dataset = SignFiCSIDataset(args.syn_dataset_dir, \
        'unconditional_cycle_target_all_syn_csi', 'unconditional_cycle_target_all_syn_label', args.normalize_csi, syn_transform)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--syn_dataset_dir', type=str, default='unconditional_synthetic_data', help='path to original data')
    ...
    # log parameters
    parser.add_argument('--log_dir', type=str, default='./unconditional_csigan_syncheck_logs', help='path to log directory')
    parser.add_argument('--checkpoint_root_dir', type=str, default='./unconditional_csigan_syncheck_checkpoints', help='path to checkpoint directory')
