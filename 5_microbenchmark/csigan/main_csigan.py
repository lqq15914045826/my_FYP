    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if epoch >= args.start_fix:
                ...
                mask_real = max_probs_real.ge(
                    args.threshold
                ).float()  # ge: greater or equal
...
def main(args):
    ...
    if args.syn_ratio != 1.0:
        cycle_target_all_syn_csi = np.load(
            os.path.join(args.syn_dataset_dir, "cycle_target_all_syn_csi.npy")
        )
        cycle_target_all_syn_label = np.load(
            os.path.join(args.syn_dataset_dir, "cycle_target_all_syn_label.npy")
        )
        selected_syn_cnt = int(args.syn_ratio * cycle_target_all_syn_csi.shape[0] / 5)
        np.random.seed(args.seed)
        selected_indices = np.random.choice(
            cycle_target_all_syn_csi.shape[0], selected_syn_cnt, replace=False
        )
        selected_syn_csi = cycle_target_all_syn_csi[selected_indices]
        selected_syn_label = cycle_target_all_syn_label[selected_indices]
        np.save(
            os.path.join(args.syn_dataset_dir, "selected_syn_csi.npy"), selected_syn_csi
        )
        np.save(
            os.path.join(args.syn_dataset_dir, "selected_syn_label.npy"),
            selected_syn_label,
        )
