...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, \
        f'{ratio_name}-{ablation_name}-{seed_name}-{data_name}-{model_name}-{threshold_name}-{temp_name}-{CURRENT_TIME}')
    ...
    if args.syn_ratio != 1.0:
        cycle_target_all_syn_csi = np.load(os.path.join(args.syn_dataset_dir, 'cycle_target_all_syn_csi.npy'))
        cycle_target_all_syn_label = np.load(os.path.join(args.syn_dataset_dir, 'cycle_target_all_syn_label.npy'))
        selected_syn_cnt = int(args.syn_ratio * cycle_target_all_syn_csi.shape[0] / 5)
        np.random.seed(args.seed)
        selected_indices = np.random.choice(cycle_target_all_syn_csi.shape[0], selected_syn_cnt, replace=False)
        selected_syn_csi = cycle_target_all_syn_csi[selected_indices]
        selected_syn_label = cycle_target_all_syn_label[selected_indices]
        np.save(os.path.join(args.syn_dataset_dir, 'selected_syn_csi.npy'), selected_syn_csi)
        np.save(os.path.join(args.syn_dataset_dir, 'selected_syn_label.npy'), selected_syn_label)
        unlabeled_syn_dataset = SignFiCSIDataset(args.syn_dataset_dir, \
            'selected_syn_csi', 'selected_syn_label', args.normalize_csi, syn_transform)
    else:
        unlabeled_syn_dataset = SignFiCSIDataset(args.ori_syn_dataset_dir, \
            'cycle_target_all_syn_csi', 'cycle_target_all_syn_label', args.normalize_csi, syn_transform)
    ...
    if args.syn_ratio != 1.0:
        os.remove(os.path.join(args.syn_dataset_dir, 'selected_syn_csi.npy'))
        os.remove(os.path.join(args.syn_dataset_dir, 'selected_syn_label.npy'))
