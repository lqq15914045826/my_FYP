...
for seed in seed_list:
    subprocess.run(["python", "onlyreal_rfdiffusion.py", \
        "--seed", str(seed), "--normalize_dfs"])

ckpt_root_dir = 'rfdiffusion_onlyreal_checkpoints'
...
ckpt_root_dir = 'rfdiffusion_reproduce_checkpoints'
ckpt_dirs = os.listdir(ckpt_root_dir)
reproduce_acc_list = []
for ckpt_dir in ckpt_dirs:
    model_ckpts = [f for f in os.listdir(os.path.join(ckpt_root_dir, ckpt_dir)) if f.endswith('.pth')]
...
ckpt_root_dir = 'rfdiffusion_filterssim_checkpoints'
ckpt_dirs = os.listdir(ckpt_root_dir)
filterssim_acc_list = []
for ckpt_dir in ckpt_dirs:
    model_ckpts = [f for f in os.listdir(os.path.join(ckpt_root_dir, ckpt_dir)) if f.endswith('.pth')]
...
ckpt_root_dir = 'rfdiffusion_filtertrts_checkpoints'
ckpt_dirs = os.listdir(ckpt_root_dir)
filtertrts_acc_list = []
for ckpt_dir in ckpt_dirs:
    model_ckpts = [f for f in os.listdir(os.path.join(ckpt_root_dir, ckpt_dir)) if f.endswith('.pth')]
