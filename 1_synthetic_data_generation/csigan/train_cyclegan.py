...
def main(args):
    ...
    checkpoint_dir = os.path.join(args.checkpoint_root_dir, 
        f'Cycle5-{scale_name}-{CURRENT_TIME}')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_data in enumerate(cycle_loader):
            ...
            # similarity loss
            loss_target_similarity = criterion_similarity(fake_target_csi, target_csi)
            loss_source_similarity = criterion_similarity(fake_source_csi, source_csi)
            # total loss
            loss_G = loss_GAN_A2B + loss_GAN_B2A + args.lambda_cycle * (loss_cycle_ABA + loss_cycle_BAB) + \
                args.lambda_similarity * (loss_target_similarity + loss_source_similarity)
            loss_G.backward()
            optimizer_G.step()
            ...
            writer.add_scalar('train_gen/loss_target_similarity', loss_target_similarity.item(), epoch * len(cycle_loader) + batch_idx)
            writer.add_scalar('train_gen/loss_source_similarity', loss_source_similarity.item(), epoch * len(cycle_loader) + batch_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_similarity', type=float, default=0.0, help='weight on similarity loss')
