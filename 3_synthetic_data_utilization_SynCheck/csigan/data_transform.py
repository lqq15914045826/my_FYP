...
class GaussianTransform(object):
    def __init__(
        self, weak_mean=0.0, weak_std=0.01, strong_mean=0.0, strong_std=0.05, seed=420
    ):
        self.seed = seed
        self.weak_mean = weak_mean
        self.weak_std = weak_std  # Standard deviation for weak augmentation
    ...
    def __call__(self, x):
        ...
        strong_noise = torch.normal(
            mean=self.strong_mean, std=self.strong_std, size=x.size()
        )
